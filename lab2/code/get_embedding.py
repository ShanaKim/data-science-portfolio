...
images_long, patches = make_data(patch_size=config["data"]["patch_size"], file_path="../data/data_labeled/*.npz")
...
for i in tqdm(range(len(images_long))):
    ...
    # save to csv
    embedding_df.to_csv(f"../data/data_labeled/image{i+1}_ae.csv", index=False)
