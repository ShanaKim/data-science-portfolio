# get embeddings for unlabeled pictures 
#for stability check and sanity check
...
model = Autoencoder_deeper(patch_size=config["data"]["patch_size"], **config["autoencoder"])
...
images_long, patches = make_data(patch_size=config["data"]["patch_size"], file_path="../data/data_test/*.npz")
...
for i in tqdm(range(len(images_long))):
    embedding_df = pd.DataFrame(embeddings[i], columns=[f"ae{i}" for i in range(embeddings[i].shape[1])])
    ...
    # save to csv
    embedding_df.to_csv(f"../data/data_test/image{i+1}_ae.csv", index=False)
